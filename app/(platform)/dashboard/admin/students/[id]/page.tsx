...
export default function StudentDetailPage({
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        `Apakah Anda yakin ingin menghapus siswa "${student?.name}"? Tindakan ini tidak dapat dibatalkan.`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      console.error("Error deleting student:", _error);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <Card>
            <CardContent className="pt-6">
              <div className="text-center">
                <div
                  className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${getGradeBadge(student.grade)}`}
                >
                  {student.grade}
                </div>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center">
                <div
                  className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${getStatusBadge(student.status)}`}
                >
          ...
          <TabsContent value="applications">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Daftar pendaftaran program yang diikuti
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                {student.messages.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {student.messages.map((message) => (
                      <div
                        key={message.id}
                        className={`p-4 border rounded-lg ${!message.read ? "bg-emerald-50 border-emerald-200" : ""}`}
                      >
