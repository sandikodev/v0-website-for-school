...
import FormsManagement from "@/components/dashboard/forms/FormsManagement";
...
type ApplicantWithSchool = FormSubmission & {
  school?: {
    name: string | null;
  } | null;
};
...
// Dynamically import TabsContent to avoid hydration mismatch
...
export default function SPMBPage() {
  ...
  const [applicants, setApplicants] = React.useState<ApplicantWithSchool[]>([]);
  const [stats, setStats] = React.useState<SubmissionStats | null>(null);
  ...
  const lastRegistrationParamRef = React.useRef<string | null>(null);

  const openModalFromRegistration = React.useCallback(
    async (registrationNumber: string) => {
      try {
        const response = await fetch(
          `/api/forms/submissions/by-registration/${encodeURIComponent(
            registrationNumber,
          )}`,
        );
        ...
        if (result.success && result.data?.id) {
          ...
          console.warn(
            "Submission not found for registration number",
            registrationNumber,
          );
        }
      } catch (error) {
        console.error("Failed to resolve registration number:", error);
      }
    },
    [],
  );
  ...
  React.useEffect(() => {
    ...
    if (registrationParam) {
      if (lastRegistrationParamRef.current !== registrationParam) {
        lastRegistrationParamRef.current = registrationParam;
        void openModalFromRegistration(registrationParam);
      }
      return;
    }

    lastRegistrationParamRef.current = null;
  ...
  }, [openModalFromRegistration, router, searchParams]);

  const openModal = (applicant: ApplicantWithSchool) => {
    setSelectedApplicantId(applicant.id);
    setIsModalOpen(true);
    // Update URL with query parameter
    const params = new URLSearchParams(window.location.search);
    if (applicant.registrationNumber) {
      params.set("registration", applicant.registrationNumber);
      params.delete("applicant");
    } else if (applicant.id) {
      params.set("applicant", applicant.id);
      ...
      >
        ...
        <TabsContent value="overview">
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-3">
                    {["reguler", "prestasi"].map((jalur) => {
                      const count = applicants.filter(
                        (a) => a.jalurPendaftaran === jalur,
                      ).length;
                      const percentage =
                        applicants.length > 0
                          ? ((count / applicants.length) * 100).toFixed(0)
                          : 0;
                      return (
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Distribusi berdasarkan gelombang
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="space-y-3">
                    {["gelombang-1", "gelombang-2", "gelombang-3"].map(
                      (gelombang) => {
                        const count = applicants.filter(
                          (a) => a.gelombangPendaftaran === gelombang,
                        ).length;
                        const percentage =
                          applicants.length > 0
                            ? ((count / applicants.length) * 100).toFixed(0)
                            : 0;
                        const label = gelombang.replace(
                          "gelombang-",
                          "Gelombang ",
                        );
                        return (
            ...
            {/* Quick Actions */}
            <Card className="bg-linear-to-r from-emerald-50 to-blue-50 border-emerald-200">
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Kelola data calon siswa yang mendaftar
                </CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Generate dan kelola laporan pendaftaran
                </CardDescription>
              </CardHeader>
                ...
                <div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id="report-period"
                        placeholder="Januari - Maret 2024"
                      />
                    </div>
                    <div className="space-y-2">
