...
export default function StudentsPage() {
  ...
  const handleDelete = async (studentId: string, studentName: string) => {
    if (
      !confirm(
        `Apakah Anda yakin ingin menghapus siswa "${studentName}"? Tindakan ini tidak dapat dibatalkan.`,
      )
    ) {
      return;
    }
            ...
            <TableBody>
              {filteredStudents.map((student) => (
                <TableRow key={student.id}>
                  ...
                  <TableCell>
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${getGradeBadgeColor(student.grade)}`}
                    >
                      {student.grade}
                    </span>
                  </TableCell>
                  <TableCell>
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusBadgeColor(student.status)}`}
                    >
